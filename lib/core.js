...
Raja.prototype.rekey = function(obj) {
	return this.key(obj.url, this.vary(obj.headers));
};
...
Resource.prototype.undepend = function(resource) {
	...
	else key = this.raja.rekey(resource);
...
Raja.prototype.headers = function(havingHeaders) {
	...
	['Content-Type', 'Vary', 'ETag', 'Accept', 'X-Grant', 'X-Right', 'X-Author'].forEach(function(name) {
		var val = hasGet && havingHeaders.get(name) || headers && headers[name];
		if (val) obj[name] = val;
	});
	return obj;
};
