...
Raja.prototype.retrieve = function(url, havingHeaders, cb) {
	...
	var key = this.key(url, this.variant(this.headers(havingHeaders)));
...
Resource.prototype.depend = function(resource) {
	...
	resource.key = this.raja.key(resource.url, this.raja.vary(resource));
...
Resource.prototype.save = function() {
	...
	if (!this.key) this.key = raja.key(this.url, raja.vary(this));
...
Resource.prototype.load = function(remote, query, opts, cb) {
	...
	if (!/^https?:/.test(remote)) remote = absolute(URL.parse(this.url), remote);
