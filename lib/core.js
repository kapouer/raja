...
Raja.prototype.create = function(obj, havingHeaders) {
	...
	if (havingHeaders) obj.headers = this.headers(havingHeaders);
	if (!obj.url) {
		if (obj.key) {
	...
	obj.key = this.key(obj.url, this.vary(obj.headers));
	...
	if (!inst) {
		...
		inst = Object.create(Resource.prototype, props);
		Object.defineProperty(inst, 'raja', {
			enumerable: false,
			value: this
		});
		debug('create unsaved', obj.key);
		this.unsaved[obj.key] = inst;
	} else {
...
Raja.prototype.retrieve = function(url, havingHeaders, cb) {
	...
	var headers = this.headers(havingHeaders);
	var key = this.key(url, this.variant(headers));
...
Resource.prototype.depend = function(resource) {
	...
	if (!(resource instanceof Resource)) resource = this.raja.create(resource);
...
Resource.prototype.save = function(cb) {
	...
	if (this.key == key) {
		raja.store.set(this.key, this, function(err, resource) {
...
Raja.prototype.headers = function(havingHeaders) {
	...
	['Content-Type', 'Vary', 'ETag', 'Accept', 'X-Grant', 'X-Right', 'X-Author'].forEach(function(name) {
...
Raja.prototype.vary = function(headers) {
	...
	headers.Vary.split(',').forEach(function(header) {
		...
		if (header == 'Content-Type') {
			var type = typeis(val, ['json', 'xml', 'html']);
...
Raja.prototype.variant = function(headers) {
	...
	if (accept) {
		var type = typeis(accept, ['json', 'xml', 'html']);
		if (type && type != 'html') variant.type = type; // store html as default
	}
...
function normalizeList(str) {
	return str.split(',').map(function(str) {return str.trim();}).sort().join(',');
}
