...
Raja.prototype.shallow = function(src, dst) {
	dst = dst || {};
	for (var key in src) {
		if (key == "get") continue; // remove me when resource is always a properly enumerable instance
		dst[key] = src[key];
	}
	return dst;
};
...
Raja.prototype.create = function(url) {
	...
	obj.get = getChild.bind(this, obj);
	return obj;
};


function getChild(resource, remote, query, opts, cb) {
