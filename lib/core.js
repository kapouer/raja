...
Raja.prototype.init = function(cb) {
	...
	.await(function(err, io, store) {
		...
		this.io = io;
		this.store = store;
		this.session = require('./store/session').bind(null, this);
		this.emit('ready');
		cb(null, this);
	}.bind(this));
};
...
Resource.prototype.depend = function(resource, havingHeaders) {
	...
	if (!(resource instanceof Resource)) {
		if (typeof resource == "string" && !havingHeaders) havingHeaders = {
			'X-Raja': this.raja.opts.namespace
		};
		resource = this.raja.create(resource, havingHeaders);
	}
