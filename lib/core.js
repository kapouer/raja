...
Raja.prototype.create = function(obj, havingHeaders) {
	...
	if (!inst.url) {
		if (inst.key) {
			inst.url = keyObj(inst.key).url;
		} else {
			throw new Error("Missing key in " + JSON.stringify(inst));
		}
	}
	inst.key = raja.key(inst.url, raja.vary(inst.headers));
	raja.store.reserve(inst);
	return inst;
};
