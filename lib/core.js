...
Raja.prototype.create = function(obj, havingHeaders) {
	...
	var headers = havingHeaders ? this.headers(havingHeaders) : obj.headers;
	if (!obj.url) {
		if (obj.key) {
	...
	obj.key = this.key(obj.url, this.vary(headers));
...
Resource.prototype.save = function(cb) {
	...
	if (this.key == key) {
		raja.store.set(this.key, this, cb);
...
Resource.prototype.copy = function(newkey, cb) {
	var raja = this.raja;
