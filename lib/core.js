...
Resource.prototype.save = function() {
	...
	else if (this.key != key) console.warn("Resource changed its key", this.key, key);
...
Raja.prototype.variant = function(headers) {
	...
	if (accept) {
		...
		if (type == 'json') return {type: type}; // no type means default type
	}
};
...
Raja.prototype.keys = function(url, obj) {
	...
	list.forEach(function(k) {
		...
		val.forEach(function(val) {
			var obj = {};
			obj[k] = val;
			keys.unshift(self.key(url, obj));
		});
	});
	if (keys.length == 0) keys.push(url);
	return keys;
};
