...
Store.prototype.store = function(key, resource, cb) {
	var raja = this.raja;
	this.SlowResource.one({key: key}, function(err, row) {
		...
		row.save(resource, function(err) {
			...
			if (resource.resources) {
				q.defer(saveRelations.bind(this), row, resource.resources, true);
			}
			if (resource.parents) {
				q.defer(saveRelations.bind(this), row, resource.parents, false);
			}
			q.awaitAll(cb);
		}.bind(this));
	}.bind(this));
...
Store.prototype.set = function(key, resource) {
	...
	if (resource.resources) {
		Object.keys(resource.resources).forEach(function(rkey) {
			...
			if (!childResource.parents[key]) {
				childResource.parents[key] = true;
			}
		});
	}
	if (resource.parents) {
		Object.keys(resource.parents).forEach(function(rkey) {
			var parentResource = cache.get(rkey) || limbo[rkey];
			if (!parentResource) {
				parentResource = raja.create({key: rkey});
				beforeSaveResource.call(parentResource);
				cache.set(rkey, parentResource);
			}
			if (!parentResource.resources) {
				parentResource.resources = {};
			}
			// do not mess with whatever was put there
			if (!parentResource.resources[key]) {
				parentResource.resources[key] = true;
			}
		});
	}
	beforeSaveResource.call(resource);
	cache.set(key, resource);
	this.limbo[key] = resource;
