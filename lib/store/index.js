...
Store.prototype.store = CacheDebounce(function(key, obj, cb) {
	...
	Resource.query().where('key', key).then().asCallback(function(err, rows) {
		...
		if (!resource) {
			...
			query = resource.$query().patch(obj);
		}
		query.then().asCallback(function(err, resource) {
			if (err) return cb(err);
...
Store.prototype.get = function(key, cb) {
	...
	Resource.query().where('key', key).first().eager(['parents', 'children']).then().asCallback(function(err, resource) {
...
Store.prototype.del = function(key, cb) {
	...
	Resource.query().where('key', key).first().then(function(resource) {
		...
		resource.delete().then().asCallback(cb);
	});
};
...
Store.prototype.invalidate = function(key, cb) {
	...
	Resource.query().patch({valid: false}).where('key', key).then().asCallback(function(err) {
...
Store.prototype.invalidateParents = function(msg, cb) {
	...
	Resource.query().where('key', key).eager('parents').then().asCallback(function(err, rows) {
		if (err) return cb(err);
		...
		Resource.query().patch({valid: false}).whereIn('key', topatch).then().asCallback(cb);
	});
};
...
function saveRelations(inst, hash, cb) {
	...
	q.awaitAll(function(err, resources) {
		...
		inst.$relatedQuery('children').unrelate().then().asCallback(function(err) {
			if (err) return cb(err);
