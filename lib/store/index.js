...
Store.prototype.reserve = function(resource) {
	var raja = this.raja;
	var key = resource.key;
	var revive = false;
	var original = this.cache.get(key);
	if (!original) {
		revive = true;
		original = this.limbo[key];
	}
	if (original) {
		raja.shallow(resource, original);
	}
	if (revive) this.cache.set(key, resource);
};
...
Store.prototype.set = function(key, resource, cb) {
	...
	if (resource.resources) {
		Object.keys(resource.resources).forEach(function(rkey) {
			...
			if (!resource) resource = raja.create({key: rkey});
			if (!resource.parents) {
				resource.parents = {};
			}
