...
Store.prototype.set = function(url, obj, cb) {
	...
	this.Resource.one({url: url}, function(err, res) {
		...
		res.save(function(err) {
			...
			if (obj.links !== undefined) saveLinks.call(this, res, obj.links, cb);
			else cb(null, res);
		}.bind(this));
	}.bind(this));
};

function saveLinks(inst, links, cb) {
	var self = this;
	var hash = {};
	...
	inst.getChildren(function(err, children) {
		...
		for (var url in hash) {
			...
			saveQueue.defer(self.set.bind(self), url);
		}
