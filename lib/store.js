...
function saveRelations(inst, resources, parents, cb) {
	var self = this;
	if (resources) this.Resources.find({url: Object.keys(resources)}).run(function(err, rows) {
		if (err) return cb(err);
		var notfound = self.raja.shallow(resources);
		...
		q.awaitAll(function(err, savedRows) {
			inst.setChildren(rows.concat(savedRows), cb);
		});
	});
	else {
