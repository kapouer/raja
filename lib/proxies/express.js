...
//var GetMw = CacheOnDemand(function(url, baseUrl, req, res, next, cb) {
var GetMw = function(url, baseUrl, req, res, next) {
	var raja = this.raja;
	raja.retrieve(url, req, function(err, resource) {
		...
		if (resource && resource.valid) {
			for (var name in resource.headers) {
				if (name in {'Content-Type':1, 'ETag':1, 'Vary':1, 'Content-Encoding':1}) {
			...
			if (ETag && req.get('If-None-Match') == ETag) {
				...
				if (resource.mtime) res.set('Last-Modified', resource.mtime.toUTCString());
			...
			BufferResponse(res, function(err, bufl) {
				...
				if (res.statusCode >= 200 && res.statusCode < 300) {
					...
					if (url != baseUrl && isResJson(res)) {
						resource.depend({url: baseUrl, headers: {
							'Content-Type': resource.headers['Content-Type'],
							'Vary': resource.headers['Vary']
						}});
					}
					var lastMod = res.get('Last-Modified');
					if (lastMod) lastMod = new Date(lastMod);
					resource.mtime = lastMod && !isNaN(lastMod.getTime()) ? lastMod : new Date();
...
function invalidateAndSend(raja, msg, headers) {
	var key = raja.key(msg.url, raja.vary(headers));
	if (key != msg.url) msg.key = key;
	raja.store.invalidate(key, function(err) {
		if (err) return raja.error(err);
		raja.send(msg);
	});
}
