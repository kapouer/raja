...
ExpressProxy.prototype.middleware = function(req, res, next) {
	...
	var raja = this.raja;
	if (req.method == "GET") {
		this.raja.get(xurl, {type: req.accepts('json') && !req.accepts('html') && 'json' || undefined}, function(err, resource) {
			...
			if (resource && resource.valid) {
				...
				BufferResponse(res, function(err, bufl) {
					...
					if (url != xurl) {
						...
						if (!resource.resources[url]) resource.resources[url] = true;
					}
...
ExpressProxy.prototype.finished = function(resource, response, buf) {
	...
	this.raja.set(resource, {type: isResJson(response) && 'json' || undefined}, this.raja.error);
};
