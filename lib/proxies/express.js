	...
	raja.retrieve(url, req, function(err, resource) {
		...
		if (resource && resource.valid) {
			...
			for (var name in resource.headers) {
				if (name in {'Content-Type':1, 'ETag':1, 'Vary':1, 'Content-Encoding':1, 'X-Grant': 1}) {
...
//			cb();
			...
			BufferResponse(res, function(err, bufl) {
				...
				debug('cache response', res.statusCode, url, res._headers);
...
ExpressProxy.prototype.middleware = function(req, res, next) {
	...
	var raja = this.raja;
	var rights = req.headers['X-Right'];
	if (rights) {
