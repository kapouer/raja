		...
		this.raja.store.get(url, function(err, resource) {
			...
			if (resource && resource.valid) {
				...
				BufferResponse(res, function(err, bufl) {
					...
					if (resources.__notcalled__) resources = undefined;
					if (err) return console.error(err); // or just return if already logged
					this.finished(url, res.statusCode, res.get('Content-Type'), bufl.slice(), resources);
				}.bind(this));
...
ExpressProxy.prototype.finished = function(url, code, mime, buf, resources) {
