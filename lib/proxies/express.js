	...
	raja.retrieve(url, res, function(err, resource) {
		...
		if (resource && resource.valid) {
			...
			if (ETag && req.get('If-None-Match') == ETag) {
				...
				if (resource.mtime) {
					res.set('Last-Modified', new Date(resource.mtime).toUTCString());
				}
			...
			// prevent a middleware from sending empty 304 body
			delete req.headers['If-None-Match'.toLowerCase()];
			delete req.headers['If-Modified-Since'.toLowerCase()];
			...
			onHeaders(res, function() {
				...
				resource.mtime = lastMod.toISOString();
			});
