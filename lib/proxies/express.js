...
ExpressProxy.prototype.middleware = function(req, res, next) {
	...
	if (req.method == "GET") {
		...
		if (isReqJson(req) || req.accepts(['json', 'html']) == 'json') variant.type = 'json';
		raja.store.get(xurl, variant, function(err, resource) {
...
function isReqJson(req) {
	...
	return req.is('json') || contentType.indexOf('/json') > 0;
}
