	...
	if (req.method == "GET") {
		this.raja.store.get(xurl, function(err, resource) {
			...
			if (resource && resource.valid) {
				...
				if (resource.etag) res.set('ETag', resource.etag);
				...
				BufferResponse(res, function(err, bufl) {
					...
					this.finished(xurl, res.statusCode, res.get('Content-Type'), res.get('ETag'), bufl.slice(), resources);
				}.bind(this));
...
ExpressProxy.prototype.finished = function(url, code, mime, etag, buf, resources) {
...
function getResource(referer, resources, remote, query, cb) {
	delete resources.__notcalled__;
	...
	this.remote.get(remote, function(err, data) {
