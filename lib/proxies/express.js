...
ExpressProxy.prototype.middleware = function(req, res, next) {
	...
	if (req.method == "GET") {
		...
		BufferResponse(res, function(err, buf) {
			...
			msg.url = url;
			if (!collectionUrl || collectionUrl != url) invalidateAndSend(raja, url, msg);
			if (collectionUrl) {
				// matches the GET request on this collection
				var cmsg = shallow(msg);
				cmsg.data = [msg.data];
				cmsg.url = collectionUrl;
				invalidateAndSend(raja, collectionUrl, cmsg);
			}
		});
		next();
	}
