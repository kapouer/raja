...
LocalProxy.prototype.get = CacheOnDemand(function(path, cb) {
...
function change(raja, path) {
	raja.store.get(path, function(err, resource) {
		if (!resource || resource.data == null) return; // nothing to invalidate
		fs.stat(path, function(err, stat) {
			if (err) return raja.error(err);
			if (stat.mtime.getTime() <= resource.mtime.getTime()) return; // still valid
			fs.readFile(path, function(err, buf) {
				if (err) return raja.error(err);
				if (raja.dataEqual(resource.data, buf)) return;
