			...
			if (resource.valid) {
				if (this.watched(path)) {
					...
					fs.stat(path, function(err, stat) {
						...
						var statnow = stat.mtime;
						if (statnow.getTime() <= Date.parse(resource.mtime)) {
							// still valid
							cb(null, resource.data);
						} else {
...
LocalProxy.prototype.load = function(path, cb) {
	...
	fs.readFile(path, function(err, buf) {
		if (err) {
			...
			raja.upsert({url: path, data: buf, builder: 'local', valid: true }, raja.error);
			cb(null, buf);
		}
	});
};
...
function changeListener(change, dir, path) {
	...
	if (!w.paths[path]) {
		debug('ignoring change, path not tracked', path);
		return;
	}
...
function unlinkListener(unlink, dir, path) {
	...
	if (!w) return console.info("no watcher exists for that directory", dir);
	if (!w.paths[path]) {
		debug('ignoring unlink, path not tracked', path);
		return;
	}
...
LocalProxy.prototype.watch = function(path) {
	...
	if (!w) {
		debug('watch dir', dir);
		w = this.watches[dir] = {
			watcher: chokidar.watch(dir, {depth: 0, persistent: true, atomic: false, followSymlinks: false})
