...
RemoteProxy.prototype.get = function(url, opts, cb) {
	...
	if (!req.maxage) req.maxage = this.opts.maxage;
...
var cachedLoad = CacheDebounce(function(key, req, cb) {
	...
	raja.retrieve(key, function(err, resource) {
		...
		if (!resource.maxage) resource.maxage = req.maxage;
		if (resource.maxage < 1000 && req.preload) {
			console.warn("Raja remote proxy preloader must not hammer below", resource.maxage, "milliseconds");
			resource.maxage = 1000;
		}
...
RemoteProxy.prototype.load = function(resource, req, cb) {
	...
	request(obj, function(err, res, body) {
		...
		if (!req.timeout && resource.maxage < Infinity) {
