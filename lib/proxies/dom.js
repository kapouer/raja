...
function DomProxy(raja, dom) {
	...
	var listener = function(msg) {
		...
		var h = this;
		if (msg.url == h.viewUrl) {
			if (msg.mtime > h.mtime)	{
				delete h.validViewHtml;
			}
		} else {
			var inst = h.pages[parent];
			if (inst && msg.mtime > inst.mtime) {
				if (msg.url == inst.authorUrl) {
					delete inst.validAuthorHtml;
				}
				// this is going to be updated by the fast path - mtime matches last update
