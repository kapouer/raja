	...
	Dom.Handler.prototype.getUsed = function(inst, req, res, cb) {
		...
		raja.retrieve(inst.user.url, req, function(err, resource) {
			...
			if (inst.page && inst.live && !inst.reloads) {
				...
				var sessionCookie = stealSessionRights(req);
				if (sessionCookie) inst.opts.cookies = sessionCookie;
...
function stealSessionRights(req) {
	var rights = req.session && req.session.rights;
	if (!rights || !rights.length) return;
	var store = req.sessionStore;
	var opts = store.opts;
	// 1) generate a new session
	var decoy = {sessionStore: req.sessionStore};
	store.generate(decoy);
	// 2) give it same rights
	decoy.session.rights = rights.slice();
	decoy.session.save();
	...
	var cookieOpts = req.session.cookie.data;
	if (cookieOpts.httpOnly) delete cookieOpts.httpOnly; // check if this is really needed
