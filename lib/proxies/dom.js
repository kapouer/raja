...
function DomProxy(raja, opts) {
	...
	function listener(msg) {
		...
		msg.parents.forEach(function(key) {
			raja.store.get(key, function(err, resource) {
				...
				if (!resource) return;
				if (msg.key == key && msg.mtime > Date.parse(resource.mtime) && (!resource.itime || msg.mtime > resource.itime)) {
	...
	function domUserPlugin(page, resource) {
		...
		page.on('busy', function() {
			if (resource.itime && resource.itime > Date.parse(resource.mtime)) {
